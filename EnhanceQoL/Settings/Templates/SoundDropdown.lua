...
if not L then
	...
	if ok and aceLocale and aceLocale.GetLocale then
		local success, localeTable = pcall(aceLocale.GetLocale, aceLocale, addonName, true)
		if success then L = localeTable end
	end
end

local LSM = LibStub and LibStub("LibSharedMedia-3.0", true)

EQOL_SoundDropdownMixin = CreateFromMixins(SettingsListElementMixin)
...
local DEFAULT_PLACEHOLDER = ResolveString("None", L and (L["soundDropdownPlaceholder"] or L["None"]), _G and _G.NONE)
local DEFAULT_PREVIEW_TOOLTIP = ResolveString("Preview Sound", L and (L["previewSound"] or L["Preview Sound"]))
local DEFAULT_LABEL = ResolveString("Sound", L and (L["Sound"] or L["sound"]) or (type(SOUND) == "string" and SOUND))
...
function EQOL_SoundDropdownMixin:OnLoad()
	SettingsListElementMixin.OnLoad(self)
	if self.NewFeature then self.NewFeature:SetShown(false) end
end

function EQOL_SoundDropdownMixin:Init(initializer)
...
function EQOL_SoundDropdownMixin:GetSetting()
...
function EQOL_SoundDropdownMixin:SetupLabel()
...
function EQOL_SoundDropdownMixin:SetupPreviewButton()
...
function EQOL_SoundDropdownMixin:SetupDropdown()
...
function EQOL_SoundDropdownMixin:BuildDropdownMenu(rootDescription)
...
function EQOL_SoundDropdownMixin:GetSoundEntries()
	local list
...
function EQOL_SoundDropdownMixin:IsValueSelected(value)
...
function EQOL_SoundDropdownMixin:GetLabelForValue(value)
...
function EQOL_SoundDropdownMixin:UpdateDropdownText()
...
function EQOL_SoundDropdownMixin:GetPlaybackChannel()
...
function EQOL_SoundDropdownMixin:RegisterSettingListener()
	local setting = self:GetSetting()
...
function EQOL_SoundDropdownMixin:EvaluateState()
...
function EQOL_SoundDropdownMixin:Release()
