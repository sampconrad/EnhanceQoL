...
local function buildDivider()
	return function()
		...
		frame.Divider = tex
		function frame:Setup(data) self.setting = data end
		return frame
	end, function(_, frame)
		frame:Hide()
		frame.layoutIndex = nil
...
function lib:AddFrame(frame, callback, default)
	...
	selection:SetScript("OnKeyDown", function(selectionFrame, key)
		...
		if key == "UP" then
			if selectionFrame.SetPropagateKeyboardInput then selectionFrame:SetPropagateKeyboardInput(false) end
		...
		elseif key == "RIGHT" then
			...
			if selectionFrame.SetPropagateKeyboardInput then selectionFrame:SetPropagateKeyboardInput(true) end
		end
	end)
...
function lib:SetFrameResetVisible(frame, showReset) State.resetToggles[frame] = not not showReset end
...
function lib:SetFrameOverlayToggleEnabled(frame, enabled)
	...
	if enabled == false and State.selectionRegistry[frame] then State.selectionRegistry[frame].overlayHidden = false end
end
